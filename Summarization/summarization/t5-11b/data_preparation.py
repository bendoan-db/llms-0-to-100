...
# MAGIC %sh mkdir -p /dbfs/tmp/ben.doan@databricks.com/review ; curl -s https://s3.amazonaws.com/amazon-reviews-pds/tsv/amazon_reviews_us_Camera_v1_00.tsv.gz | gunzip > /dbfs/tmp/ben.doan@databricks.com/review/amazon_reviews_us_Camera_v1_00.tsv

# COMMAND ----------

camera_reviews_df = spark.read.options(delimiter="\t", header=True).\
  csv("/tmp/ben.doan@databricks.com/review/amazon_reviews_us_Camera_v1_00.tsv")
...
camera_reviews_df.select("product_id", "review_body", "review_headline").\
  ...
  write.format("delta").save("/tmp/ben.doan@databricks.com/review/cleaned")

# COMMAND ----------

camera_reviews_cleaned_df = spark.read.format("delta").load("/tmp/ben.doan@databricks.com/review/cleaned").\
...
train_df.toPandas().to_csv("/dbfs/tmp/ben.doan@databricks.com/review/camera_reviews_train.csv", index=False)
val_df.toPandas().to_csv("/dbfs/tmp/ben.doan@databricks.com/review/camera_reviews_val.csv", index=False)
